...
export class CodeMirrorEditor extends Disposable {
  constructor(cm) {
    ...
    this.cm = cm;
    this.registerCommand("done", () => this.markDone());
  }

  markDone() {
    const lineNumber = this.cm.doc.getCursor().line;
    const line = this.cm.doc.getLine(lineNumber);
    ...
    replaceRules.forEach(([regex, replacement]) => {
      ...
      this.cm.doc.replaceRange(
        replacedLine,
        new Pos(lineNumber, 0),
        new Pos(lineNumber, line.length)
      );
      return true;
    });
  }
